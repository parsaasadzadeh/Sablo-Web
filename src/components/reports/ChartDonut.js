...
export default function ChartDonut({ summary, display, unit }) {
  const [activeSlice, setActiveSlice] = useState(null);

  const data = SLICES
    .filter((s) => summary[s.key] > 0)
    .map((s) => ({ ...s, value: summary[s.key] }));
  ...
  if (data.length === 0 || total === 0) {
    return (
      <div className="bg-white rounded-2xl border border-[#EDE8DC] p-6 mb-6">
        <h3 className="text-base font-bold text-[#26241F] mb-4 text-right">توزیع مالی</h3>
        <p className="text-sm text-[#8A8273] text-center py-10">هنوز تراکنشی ثبت نشده</p>
      </div>
    );
  }
